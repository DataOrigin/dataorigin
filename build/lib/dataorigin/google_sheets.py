...
def _load_credentials(scopes: List[str] = SCOPES):
    ...
    3. OAuth2 de usuario: token inline en GOOGLE_OAUTH_TOKEN_JSON, o archivo en
       GOOGLE_OAUTH_TOKEN_FILE (default token.json); si no hay token valido y el
       entorno es interactivo, flow via GOOGLE_OAUTH_CLIENT_SECRET_FILE u
       OAUTH2_CLIENT_ID/OAUTH2_CLIENT_SECRET. En entornos headless (sin TTY o con
       GOOGLE_AUTH_NO_INTERACTIVE=1) nunca se abre navegador: se lanza RuntimeError.
    ...
    token_json = (os.getenv("GOOGLE_OAUTH_TOKEN_JSON") or "").strip()
    ...
    if client_secret_file or has_oauth_env or token_json:
        creds: Optional[UserCredentials] = None
        token_from_env = False

        # Intentar cargar token inline (GOOGLE_OAUTH_TOKEN_JSON) o desde archivo
        if token_json:
            try:
                creds = UserCredentials.from_authorized_user_info(json.loads(token_json), scopes)
                token_from_env = True
            except Exception:
                raise RuntimeError("Invalid GOOGLE_OAUTH_TOKEN_JSON (must be the JSON content of an authorized user token)")
        ...
        if not creds or not creds.valid:
            ...
            if not creds:
                if os.getenv("GOOGLE_AUTH_NO_INTERACTIVE") or not sys.stdin.isatty():
                    raise RuntimeError(
                        "No valid Google OAuth token and interactive auth is not possible "
                        "(headless environment). Regenerate the token on a machine with a "
                        "browser and distribute it via GOOGLE_OAUTH_TOKEN_FILE or GOOGLE_OAUTH_TOKEN_JSON."
                    )
            ...
            # Guardar token para uso futuro (solo si vino de archivo; el inline vive en el env)
    ...
    raise RuntimeError("Set GOOGLE_SERVICE_ACCOUNT_JSON or GOOGLE_APPLICATION_CREDENTIALS or GOOGLE_OAUTH_TOKEN_JSON or GOOGLE_OAUTH_CLIENT_SECRET_FILE or OAUTH2_CLIENT_ID/OAUTH2_CLIENT_SECRET")
